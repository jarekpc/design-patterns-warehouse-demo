...
public class Warehouse {
    ...
    public static final String DEFAULT_PRODUCTS_CSV_FILE = "products.csv";
    public static final String DEFAULT_INVENTORY_CSV_FILE = "inventory.csv";
    public static final String DEFAULT_CUSTOMERS_CSV_FILE = "customers.csv";
    public static final String DEFAULT_ORDERS_CSV_FILE = "orders.csv";
    ...
    private void readProducts() throws FileNotFoundException, WarehouseException {
        final CsvReader reader = new CsvReader(new FileInputStream(DEFAULT_PRODUCTS_CSV_FILE));
    ...
    private void readInventory() throws FileNotFoundException, WarehouseException {
        final CsvReader reader = new CsvReader(new FileInputStream(DEFAULT_INVENTORY_CSV_FILE));
        while (reader.hasNextRow()) {
            final List<String> row = reader.nextRow();
            if (row.isEmpty()) {
                continue;
            ...
            try {
                ...
                throw new WarehouseException("Failed to read Inventory: invalid product ID in CSV, must be a integer.", ex);
            }
            if (!products.containsKey(id)) {
            ...
            try {
                ...
                throw new WarehouseException("Failed to read inventory: invalid quality in CSV, must be an integer.", ex);
            }
    ...
    private void readCustomers() throws FileNotFoundException, WarehouseException {
        final CsvReader reader = new CsvReader(new FileInputStream(DEFAULT_CUSTOMERS_CSV_FILE));
        while (reader.hasNextRow()) {
            final List<String> row = reader.nextRow();
            if (row.isEmpty()) {
                continue;
            ...
            try {
                ...
                throw new WarehouseException("Failed to read customers: invalid customer ID in CSV, must be a integer.", ex);
            }
            final String name = row.get(1);
    ...
    private void readOrders() throws FileNotFoundException, WarehouseException {
        final CsvReader reader = new CsvReader(new FileInputStream(DEFAULT_ORDERS_CSV_FILE));
        while (reader.hasNextRow()) {
            final List<String> row = reader.nextRow();
            if (row.isEmpty()) {
                continue;
            ...
            try {
                ...
                throw new WarehouseException("Failed to read orders: invalid order ID in CSV, must be integer", ex);
            }
            int customerId;
            try {
                customerId = Integer.valueOf(row.get(1));
            } catch (NumberFormatException ex) {
                throw new WarehouseException("Failed to read orders: invalid customer ID in CSV, must be integer", ex);
            }

            final Customer customer = customers.get(customerId);
            if (customer == null) {
                throw new WarehouseException("Failed to read orders: unknow customer ID in CSV");
            }
            LocalDate date;
            try {
                date = LocalDate.parse(row.get(2));
            } catch (NumberFormatException ex) {
                throw new WarehouseException("Failed to read orders: invalid date in CSV, format must by 'yyyy-MM-dd'", ex);
            }
            final Boolean pending = Boolean.valueOf(row.get(3));
            Map<Product, Integer> quantities = new HashMap<>();
            for (String orderDetail : row.subList(4, row.size())) {
                final String[] productIdAndQuantity = orderDetail.split("x");
                int productId;
                try {
